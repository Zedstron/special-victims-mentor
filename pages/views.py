...
def upload(request):
    ...
    try:
        if request.method == 'POST' and request.FILES['file']:
            ...
            result = GetResult("./media/{}".format(filename), "VertexAI")
            context["url"] = fs.url(filename)
            context["message"] = result
            context["status"] = True if result else False

            if context["status"]:
                try:
                    for event in context["message"]:
                        event['request'] = request
                        CreateEvent(**event)
                        context["eventsError"] = False
                except Exception as e:
                    print(e)
                    context["eventsError"] = True
        else:
